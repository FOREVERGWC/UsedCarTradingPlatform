<template>
  <div>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-select v-model="queryParams.userId" clearable filterable placeholder="请选择卖方">
                <el-option v-for="item in userList" :key="item.id" :label="item.name" :value="item.id"/>
              </el-select>
            </el-col>
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-select v-model="queryParams.auditorId" clearable filterable placeholder="请选择审核员">
                <el-option v-for="item in userList" :key="item.id" :label="item.name" :value="item.id"/>
              </el-select>
            </el-col>
            <el-col :lg="4" :md="4" :sm="12" :xl="4" :xs="12">
              <el-input v-model="queryParams.info" clearable placeholder="请输入配置"/>
            </el-col>
      ...
      <el-col :span="24">
        <el-card>
          <el-row :gutter="20">
            ...
            <el-col :lg="2" :md="2" :sm="12" :xl="2" :xs="12">
              <vue3-json-excel :fields="carAuditeFields" :json-data="carAuditeList" name='二手车审核列表.xls'>
    ...
    <el-card>
      <el-table v-loading="loading" :cell-style="{ textAlign: 'center' }" :data="carAuditeList"
                :header-cell-style="{ textAlign: 'center' }" stripe
                @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55"/>
        <el-table-column label="序号" type="index" width="70"/>
        <el-table-column label="车辆ID" prop="carId"/>
        <el-table-column label="卖方ID" prop="userId"/>
        <el-table-column label="审核员ID" prop="auditorId"/>
        <el-table-column label="配置" prop="info"/>
    ...
    <el-dialog :title="form.title" v-model="form.visible" destroy-on-close width="40%">
      <el-form ref="formRef" :model="form.data" :rules="rules" label-width="80px">
        ...
        <el-form-item label="配置" prop="info">
...
import {getCarAuditeOne, getCarAuditePage, removeCarAuditeBatchByIds, saveCarAudite} from '@/api/carAudite.js'
...
const total = ref(0)
const carAuditeFields = {
  '序号': {
    field: 'id',
    callback: (id) => {
      const index = carAuditeList.value.findIndex(item => item.id === id)
      return index + 1
    }
  },
  '主键ID': 'id',
...
const rules = {
  ...
  userId: [{required: true, message: '请输入卖方ID', trigger: 'blur'}],
  auditorId: [{required: true, message: '请输入审核员ID', trigger: 'blur'}],
  info: [{required: true, message: '请输入配置', trigger: 'blur'}]
}
...
const getPage = () => {
  ...
  getCarAuditePage(queryParams).then(res => {
    carAuditeList.value = res.data?.records || []
...
const showEdit = (row) => {
  ...
  getCarAuditeOne(params).then(res => {
...
const handleSave = () => {
  formRef.value.validate(valid => {
    if (!valid) return
    saveCarAudite(form.value.data).then(res => {
      if (res.code !== 200) {
...
const handleDelete = (id) => {
  ...
  removeCarAuditeBatchByIds(params).then(res => {
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
